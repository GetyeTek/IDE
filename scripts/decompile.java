// Native Java script to decompile functions in chunks of 3000 and output to .txt files
// @category Decompiler
...
public class decompile extends GhidraScript {
    ...
    public void run() throws Exception {
        ...
        println("[+] Commencing chunked decompilation (3000 functions per file)... ");

        try {
            while (funcs.hasNext() && !monitor.isCancelled()) {
                Function func = funcs.next();
                
                // Trigger file rotation at the start and after every 3000 functions
                if (count % 3000 == 0) {
                    if (writer != null) {
                        writer.close();
                    }
