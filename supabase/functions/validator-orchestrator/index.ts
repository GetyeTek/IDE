...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'get_work') {
      ...
      const { count: totalCount, error: countErr } = await supabase.from('validation_tracking_sf').select('*', { count: 'exact', head: true });
      ...
      if (countErr) {
        ...
        const { data: files, error: listErr } = await supabase.storage.from('inspection_bucket').list('small_families', { limit: 1000 });
        ...
        const filePaths = (files ||[])
          ...
          .map(f => ({
            file_path: 'small_families/' + f.name,
            status: 'pending',
        ...
        if (filePaths.length > 0) {
          ...
          const { error: upsertErr } = await supabase.from('validation_tracking_sf').upsert(filePaths, { onConflict: 'file_path', ignoreDuplicates: true });
          if (upsertErr) {
      ...
      const { data: rpcData, error: rpcErr } = await supabase.rpc('claim_batch_sf', { worker_id_param: worker_id });
