...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'get_work') {
      ...
      const { count: totalCount, error: countErr } = await supabase.from('validation_tracking_orphan_rev').select('*', { count: 'exact', head: true });
      ...
      if (countErr) {
        ...
        const { data: files, error: listErr } = await supabase.storage.from('inspection_bucket').list('orphan_rev', { limit: 1000 });
        ...
        if (filePaths.length > 0) {
          ...
          const { error: upsertErr } = await supabase.from('validation_tracking_orphan_rev').upsert(filePaths, { onConflict: 'file_path', ignoreDuplicates: true });
          if (upsertErr) {
      ...
      const { data: rpcData, error: rpcErr } = await supabase.rpc('claim_batch_orphan_rev', { worker_id_param: worker_id });
